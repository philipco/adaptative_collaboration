...
class Client:
    ...
    def set_step_scheduler(self, dataset_name, scheduler_steps, scheduler_gamma):
        ...
        elif dataset_name in ["Y"]:
            self.scheduler = ReduceLROnPlateau(self.optimizer, 'min', patience=4)
            if self.algo_name in ["Ditto", "Apfl"]:
                self.global_scheduler = ReduceLROnPlateau(self.global_optimizer, 'min', patience=4)
            if self.algo_name in ["Apfl"]:
                self.personalized_scheduler = ReduceLROnPlateau(self.personalized_optimizer, 'min', patience=4)
        else:
